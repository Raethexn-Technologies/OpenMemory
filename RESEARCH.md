...
**Source-backed evidence facts implemented (2026-06-02).** Document ingestion now runs `EvidenceFactExtractionService` after each chunk node is stored. The service extracts small source-backed facts from the chunk, asks the model for an exact supporting quote, and computes character spans locally by locating that quote in the redacted chunk. Facts are stored in `evidence_facts` with `source_node_id`, `source_document_id`, `span_start`, `span_end`, confidence, and chunk metadata. This does not reproduce recursive token lineage down to words and characters. It does create the first source-span primitive needed for grounded document QA and later claim verification.

**Grounded document QA implemented behind a flag (2026-06-02).** Setting `GROUNDED_RETRIEVAL=true` makes the chat path use graph retrieval as a candidate generator, then `EvidenceRetrievalService` selects matching public document facts against the current user query. `LlmService::buildGroundedSystemPrompt()` instructs the model to answer only from those facts, cite `[EVID:<id>]`, and refuse when the evidence does not support the question. In grounded mode, the model receives only the current redacted user question and the selected facts, not the prior chat history. This is a constrained prompt architecture, not an attention-bypass architecture.
...
**Source-agnostic wiring confirmed (2026-04-09).** The `graph:coherence-check` artisan command injected a synthetic document chunk node with the four most common tags from existing chat nodes (`artificial intelligence, software development, tools, engineering`) and confirmed that a `same_topic_as` edge formed at weight 1.0 to the chat node "Anthony builds AI tools". The wiring layer does not branch on source. Cross-source connections depend on extraction vocabulary consistency, not graph structure. See DEVLOG Entry 022.

The remaining open question is whether `GraphExtractionService` produces overlapping tags for real document content and real chat turns on the same topic. That is a vocabulary measurement, not a wiring fix. It is testable by ingesting a real document and observing the extracted tags against those already in the graph.
...
**Goal-biased retrieval implemented (2026-04-09).** `MemoryGraphService::findContextSeeds()` now seeds all public `goal` nodes first, then fills the remaining seed slots from the same 60-node bounded candidate pool ranked by total connected edge weight. Goal IDs are excluded from the weighted pool to prevent duplication. Retrieval is no longer purely history-weighted. It is now goal-biased.

**Query-aware retrieval implemented (2026-07-16).** The limitation recorded above is closed. `query_graph` seeds BFS from nodes ranked by deterministic lexical relevance to the current redacted user message, and `hybrid_query_graph` combines normalized query relevance (0.5), accumulated edge weight (0.3), and recency (0.2), admitting goal seeds only when the goal is lexically relevant to the message. Both strategies share the BFS expansion and its sensitivity filters with the existing strategies. The measurement is recorded under Track 10 Claim 1.
...
Claim 1 (better retrieval) was not confirmed by those runs. The recency advantage shrank on harder corpora but did not flip at CONTEXT_LIMIT=12.

**Query-awareness hypothesis (stated 2026-07-16, before measurement).** The 2026-04-22 runs compared strategies that never see the question being judged. The seeds came from global goal nodes and accumulated edge weight alone, so the recorded recency advantage is the expected outcome for a query-blind system, not evidence against graph retrieval as such. Three falsifiable hypotheses follow. H1: seeding BFS from nodes lexically relevant to the current query (`query_graph`) closes most or all of the composite gap to recency, because query-blind seed selection is a major source of the deficit. H2: goal seeding helps only planning-type questions, so a hybrid that admits goal seeds only when the goal is lexically relevant to the query keeps most of the goal-alignment benefit while avoiding the composite penalty on historical-fact questions. H3 states the negative branch: if `query_graph` still loses to recency on composite with complete judge calls, query-blindness does not explain the deficit and the next suspect is the tag-overlap edge vocabulary.

The experiment: `benchmark:retrieval` with strategies recency, graph, goal_graph, query_graph, hybrid_query_graph across corpora 01 through 05 at CONTEXT_LIMIT=12. Corpus 05 is new: 43 memories plus 3 goals spanning 14 months, every question labeled with a class (planning, historical_decision, cross_topic_synthesis, contradiction_resolution, recent_status, durable_preference, insufficient_evidence), and the recent window deliberately dominated by routine operational notes so recency-leaning retrieval pays a measurable price on questions about older knowledge. A deterministic theme-coverage metric (lexical presence of expected themes in retrieved context) runs alongside the LLM judge, and every judged result stores a retrieval trace. The judge model identifier is recorded in the output.

**Status: measured, query-awareness supported, graph-value subclaim still open (2026-07-16).** The first query-aware runs used five strategies and did not include a query-aware non-graph baseline. A later complete six-strategy run added that control and is now the relevant result for this claim. Raw output: `storage/benchmarks/results-2026-07-16_193025.json` (complete run, 192/192 judge calls, judge model anthropic/claude-sonnet-4.5). Earlier incomplete six-strategy attempts are preserved in `storage/benchmarks/` and correctly marked incomplete by the report.

Aggregate composite across 32 questions (complete run): recency 2.52, graph 2.55, goal_graph 2.55, query_lexical 3.56, query_graph 3.55, hybrid_query_graph 3.52. Query-aware retrieval clearly beats recency and the query-blind graph strategies on this synthetic context benchmark: query_lexical +41.3% over recency, query_graph +40.9%, hybrid_query_graph +39.7%. Deterministic theme coverage orders the query-aware strategies above the query-blind strategies as well: query_lexical 0.71, query_graph 0.72, hybrid_query_graph 0.69, recency 0.48, graph 0.48, goal_graph 0.44.

The lexical-only control changes the interpretation. query_graph changed composite by -0.3% relative to query_lexical, while adding about 8 graph-expanded nodes and 85 more estimated context tokens on average. Original-corpora composites were query_lexical 3.60, query_graph 3.64, hybrid_query_graph 3.80. On the new adversarial long-horizon corpus, query_lexical led query_graph 3.50 to 3.40 and hybrid_query_graph 3.06. The current evidence therefore supports query-aware lexical relevance as the main measured improvement; it does not show reliable added value from BFS graph traversal after lexical selection has already chosen good candidates.

H1 is supported in the narrower form: query-blind seed selection was a major cause of the earlier composite deficit, and query-aware retrieval substantially improved context quality. H1 is not evidence that the graph traversal itself is beneficial. H2 remains directional rather than established: on the two planning questions, goal_graph still leads (4.13), hybrid_query_graph scores 3.50, query_lexical 3.38, and query_graph 3.13; on three historical-decision questions, query_graph scores 4.08 while query_lexical scores 3.00 and goal_graph 1.17. On cross-topic synthesis, query_lexical leads query_graph 4.00 to 3.58. Those class slices are useful diagnostics but the samples are still too small for a strong per-class claim.

H3 is not triggered against recency, because query-aware retrieval beats recency. The tag-overlap vocabulary question remains open, and the graph-value question remains open for the stronger reason that query-aware graph retrieval has not beaten the query-aware lexical-only baseline.

Claim 1 is now supported at the context level for query-aware retrieval on synthetic corpora. The graph-value subclaim remains unproven.
...
**New answer-level path available (2026-06-02).** Grounded document QA gives Claim 1 a second evaluation target. Instead of scoring only retrieved memory nodes, a benchmark can now ingest documents, extract evidence facts, ask document-scoped questions with `GROUNDED_RETRIEVAL=true`, and score the final answer for evidence coverage, citation accuracy, unsupported claim rate, and false refusal rate. This should become a separate benchmark from `benchmark:retrieval` because it measures answer behavior, not context selection alone.
