...
**Memory graph with Physarum dynamics.** A PostgreSQL graph layer stores each confirmed memory as a typed node (memory, person, project, document, task, event, concept, goal) with directed semantic edges (same_topic_as, about_person, part_of, caused_by, related_to). Edge weights evolve according to the Tero et al. (2010) discrete Physarum polycephalum model: weights increment by ALPHA=0.10 on Hebbian co-activation (two nodes loaded into the same LLM context window count as one co-activation event) and decay by RHO=0.97 daily. The weight floor is 0.05; edges never fully disappear. This turns the graph into a relevance index: nodes and edges that the LLM has found useful together accumulate weight; unused paths decay toward the floor and become traversal dead-ends.

**Graph-guided retrieval replacing flat recall.** LLM context assembly uses the Physarum neighbourhood of a seed set rather than the full public memory set. Six retrieval strategies exist, selected by configuration: recency, weight-ranked, goal-biased, query-aware lexical-only, query-aware graph, and a hybrid that combines normalized query relevance, edge weight, and recency with goals admitted only when query-relevant. BFS from graph seeds collects neighbours in weight-descending order up to a limit, with the public-only and unconsolidated filters applied at every hop. Only the retrieved neighbourhood is reinforced on each graph turn, so frequently traversed paths grow stronger and unused paths decay toward the floor. Cold-start fallback to flat ICP recall handles the case where no graph exists yet. Each turn's response now includes `active_node_ids`, the exact node IDs that entered the context window. The 2026-07-16 benchmark established that query-conditioned retrieval is much stronger than query-blind seed selection on the synthetic corpora. A later complete run with the lexical-only control showed `query_graph` effectively tied with `query_lexical` (3.55 vs 3.56 composite), so it does not yet prove that graph traversal adds value beyond lexical selection. The stable conclusion is narrower: usage weight and goals alone do not compensate for ignoring the question, lexical relevance explains most of the measured gain, and the graph-value hypothesis remains open.
...
**The graph wiring is source-agnostic when tags match.** A synthetic coherence check inserted a document-sourced chunk using the most common tags from existing chat nodes and observed a `same_topic_as` edge at weight 1.0 to a chat node. The wiring layer does not branch on `source`. The remaining open measurement is whether the extractor produces overlapping tags for real document content and real chat turns on the same topic.
